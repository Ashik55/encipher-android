...
Client request the sign-up flows, once the homeserver is chosen by the user and its url is known (in the example it's `https://encipher.kainotomia.tech`)

```shell script
curl -X POST --data $'{}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
```shell script
curl -X POST --data $'{"initial_device_display_name":"Mobile device","username":"alice","password": "weak_password"}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
```shell script
curl -X POST --data $'{"auth":{"session":"xptUYoREDACTEDogOWAGVnbJQ","type":"m.login.dummy"}}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
```shell script
curl -X POST --data $'{"client_secret":"53e679ea-oRED-ACTED-92b8-3012c49c6cfa","email":"alice@email-provider.org","send_attempt":0}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register/email/requestToken'
```
...
```shell script
curl -X POST --data $'{"auth":{"threepid_creds":{"client_secret":"53e679ea-oRED-ACTED-92b8-3012c49c6cfa","sid":"qlBCREDACTEDEtgxD"},"session":"xptUYoREDACTEDogOWAGVnbJQ","type":"m.login.email.identity"}}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
User clicks on the link received by email `https://encipher.kainotomia.tech/_matrix/client/unstable/registration/email/submit_token?token=vtQjQIZfwdoREDACTEDozrmKYSWlCXsJ&client_secret=53e679ea-oRED-ACTED-92b8-3012c49c6cfa&sid=qlBCREDACTEDEtgxD` which contains:
...
```shell script
curl -X POST --data $'{"auth":{"session":"xptUYoREDACTEDogOWAGVnbJQ","type":"m.login.terms"}}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
```shell script
curl -X POST --data $'{"client_secret":"d3e285f6-972a-496c-9a22-7915a2db57c7","send_attempt":1,"country":"FR","phone_number":"+33611223344"}'  'https://encipher.kainotomia.tech/_matrix/client/r0/register/msisdn/requestToken'
```
...
```shell script
curl -X POST --data $'{"auth":{"type":"m.login.msisdn","session":"xptUYoREDACTEDogOWAGVnbJQ","threepid_creds":{"client_secret":"d3e285f6-972a-496c-9a22-7915a2db57c7","sid":"1678881798"}}}' 'https://encipher.kainotomia.tech/_matrix/client/r0/register'
```
...
```shell script
curl -X POST --data $'{"client_secret":"d3e285f6-972a-496c-9a22-7915a2db57c7","sid":"1678881798","token":"123456"}' 'https://encipher.kainotomia.tech/_matrix/client/unstable/add_threepid/msisdn/submit_token'
```
